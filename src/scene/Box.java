...
public class Box {
	...
	// ---------------------------------operations-----------------------------------
	/**
	 * This function return the maximum indexes for mapping the geometry into
	...
	private int[] getMaxIndexs(Point3D max) {
		...
		maxIndexs[0] = (int) ((max.getX() - _minX) / _voxelSizeX);
		maxIndexs[1] = (int) ((max.getY() - _minY) / _voxelSizeY);
		maxIndexs[2] = (int) ((max.getZ() - _minZ) / _voxelSizeZ);
		return maxIndexs;
	}
	...
	private int[] getMInIndexs(Point3D min) {
		...
		minIndexs[0] = (int) ((min.getX() - _minX) / _voxelSizeX);
		minIndexs[1] = (int) ((min.getY() - _minY) / _voxelSizeY);
		minIndexs[2] = (int) ((min.getZ() - _minZ) / _voxelSizeZ);
		return minIndexs;
	}
	 ...
	 * 3DDDA algorithm *** case of shadow rays we need to traverse all the voxels
	...
	private static class Voxel {
		...
		private static Voxel convertPointToVoxel(Point3D point) {
			int x = (int) ((point.getX() - _minX) / _voxelSizeX);
			int y = (int) ((point.getY() - _minY) / _voxelSizeY);
			int z = (int) ((point.getZ() - _minZ) / _voxelSizeZ);
