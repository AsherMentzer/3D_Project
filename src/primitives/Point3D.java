...
/**
 * class Point3d is base class to represent 1 point in 3d with 3 coordinate
 * 
...
public final class Point3D {
	...
	public Point3D add(Vector v) {
		...
		return new Point3D(new Coordinate(this.x.coord + head.x.coord), new Coordinate(this.y.coord + head.y.coord),
				new Coordinate(this.z.coord + head.z.coord));
	}
	...
	public double distanceSquared(Point3D p) {
		return ((p.x.coord - this.x.coord) * (p.x.coord - this.x.coord)
				+ (p.y.coord - this.y.coord) * (p.y.coord - this.y.coord)
				+ (p.z.coord - this.z.coord) * (p.z.coord - this.z.coord));
	}
